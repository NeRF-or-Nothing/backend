...
def run_full_sfm_pipeline(id, video_file_path, input_data_dir, output_data_dir, use_gpu="false"):
    ...
    logger.debug(f"Video background is {'white' if is_white_background else 'not white'}")

    # (1) vid_to_images.py
    imgs_folder = os.path.join(output_path, "imgs")
    logger.debug("Video file path:{}".format(video_file_path))

    split_status = split_video_into_frames(video_file_path, imgs_folder, 200)
    ...
    status = run_colmap(colmap_path, imgs_folder, output_path, use_gpu)
    if status == 0:
...
def colmap_worker():
    ...
    def process_colmap_job(ch, method, properties, body):
        ...
        motion_data["sfm"] = {
            "intrinsic_matrix": motion_data.pop("intrinsic_matrix"),
            "frames" : motion_data.pop("frames"),
            "white_background": motion_data.pop("white_background")   
        }

        logger.info("Sending motion data to rabbitmq: {}".format(motion_data))
